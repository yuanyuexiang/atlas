...
class RAGAgent:
    ...
    def _create_agent(self):
        ...
        self.agent = create_agent(
            ...
            工作流程：
            1. 先使用 knowledge_base_search 工具检索相关文档
            2. **仔细阅读检索结果，提取关键信息**
            3. **基于检索到的内容，用自己的话组织答案**
            4. 回答要简洁、准确、有针对性

            重要规则：
            - ❌ 不要直接返回工具的原始输出（不要直接返回"文档片段1"、"文档片段2"）
            - ✅ 必须提取、总结、归纳检索到的信息，生成自然流畅的回答
            - ✅ 如果知识库中没有答案，诚实告知用户
            - ✅ 回答要像人一样自然对话，不要生硬复制粘贴

            示例：
            用户问："宋江有什么兵器？"
            ❌ 错误回答：直接返回"文档片段1: ...宋江...武松..."
            ✅ 正确回答："根据《水浒传》的记载，宋江主要使用的兵器是双刀。在书中多次提到宋江善使双刀，并以此在江湖上闯出名声。"""
        )
