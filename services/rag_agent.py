...
class RAGAgent:
    ...
    def _create_agent(self):
        ...
        def knowledge_base_search(query: str) -> str:
            ...
            Returns:
                知识库中与查询最相关的文档片段（原始数据，需要你提取、总结后回答用户）
            """
            results = milvus_store.search_similar(agent_name, query, top_k=3)
            if not results:
                return "未找到相关内容。请告诉用户：抱歉，我在知识库中没有找到相关信息。"
            
            # 返回纯文本内容，不加"文档片段"标签（避免 Agent 误以为是最终答案）
            context_parts = []
            for i, r in enumerate(results[:3], 1):
                context_parts.append(f"[检索结果 {i}]\n{r['content']}\n")
            
            raw_context = "\n".join(context_parts)
            
            # 添加提示，告诉 Agent 这是原始数据
            return f"""以下是从知识库检索到的相关信息（原始数据）：
...
你是一个智能助手，擅长从知识库中检索信息并用自然语言回答用户问题。

**关键要求（必须遵守）：**

1. 当用户提问时，使用 knowledge_base_search 工具检索相关信息
2. **收到工具返回结果后，你必须继续思考和处理：**
   - 仔细阅读检索到的文档片段
   - 提取其中与问题相关的关键信息
   - 用简洁、通俗的语言重新组织答案
   - **绝对不要直接返回"文档片段1"、"文档片段2"这样的原始内容**

3. 回答格式要求：
   - ✅ 正确：直接回答问题，像正常对话一样
   - ❌ 错误：返回"文档片段1: xxx"、"文档片段2: xxx"
...
✅ 正确（必须这样回答）：
"根据知识库的资料，XXX 主要有以下特点：[提取的关键信息]。具体来说，[用自己的话总结]。"

用户问："你能提供哪些信息？"
✅ 正确回答：
"我可以根据知识库为您提供相关领域的信息。请告诉我您想了解什么具体内容，我会为您查找并解答。"

**记住：你的任务是理解文档内容后，用自然流畅的语言回答用户，而不是复制粘贴原始文档！**"""
        )
