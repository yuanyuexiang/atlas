...
> 基于 **FastAPI + PostgreSQL + Milvus + LangChain** 构建的智能客服 RESTful API 后端系统
...
Agent {
    ...
    display_name: "法律顾问"    # 显示名称
    agent_type: "legal"         # 类型：general/legal/medical/financial
    system_prompt: "你是..."    # 系统提示词（定义角色）
    status: "active"            # 状态：active/inactive/training
...
```python
Document {
    file_id: "doc_20250119_001"    # 文档 ID
    filename: "minfadian.pdf"      # 文件名
    agent_name: "legal-advisor"    # 所属智能体
    chunks_count: 1250             # 切分的片段数
...
#### 1. 向量检索优化
- **索引类型**: IVF_FLAT（适合百万级向量）
- **相似度算法**: 余弦相似度（Cosine Similarity）
- **Top-K 限制**: 默认检索前 4 个最相关片段
- **缓存策略**: 常见问题结果缓存（可选）
...
Echo 采用创新的三层解耦设计，实现了界面、能力和数据的完全分离：
...
├── models/                     # 数据模型层
│   ├── __init__.py
│   ├── entities.py             # 数据库实体（Agent/Conversation）
│   ├── auth.py                 # 用户实体模型
...
│   ├── multi_rag_manager.py    # RAG 管理器（知识库）
│   └── rag_agent.py            # RAG 智能体实现
│
├── core/                       # 核心配置
│   ├── __init__.py
│   ├── config.py               # 应用配置
